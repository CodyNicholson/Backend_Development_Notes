...
ref_index= 0
	shell_command= 'py input-writer.py 0 0 ' + input_files[input_index] + ' | java InputProgram >output.txt'
...
for input_index in range(size):
	shell_command= 'py input-writer.py 0 0 blank.txt | java InputProgram ' + option_args[input_index] + ' >output.txt'
	os.system( shell_command )
	compare= FileCompare()
	differ= compare.textFiles( 'output.txt', ref_files[ref_index], True )
	if differ is False:
...
for input_index in range(size):
	shell_command= 'py input-writer.py 0 0 ' + input_files[input_index] + ' | java InputProgram ' + option_args[input_index] + ' >output.txt'
	os.system( shell_command )
	compare= FileCompare()
	differ= compare.textFiles( 'output.txt', ref_files[ref_index], True )
	if differ is False:
